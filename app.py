...
with tab1:
    ...
    if dir1 == "平面直角 → 緯度経度":
        ...
        for i, pt in enumerate(pts):
            ...
            with c0:
                st.markdown(f"<div style='padding-top:{'32' if i==0 else '8'}px;font-size:12px;font-weight:700;color:#64748b'>#{i+1}</div>",
                            unsafe_allow_html=True)
            with c1:
                pt["name"] = st.text_input("点名", value=pt["name"], key=f"jpc_nm_{i}",
                                           label_visibility="visible" if i==0 else "collapsed")
            with c2:
                pt["x"] = st.text_input("X 北が正 (m)" if i==0 else "X (m)",
                                        value=pt["x"], placeholder="-42090.367",
                                        key=f"jpc_x_{i}",
                                        label_visibility="visible" if i==0 else "collapsed")
            with c3:
                pt["y"] = st.text_input("Y 東が正 (m)" if i==0 else "Y (m)",
                                        value=pt["y"], placeholder="-23809.574",
                                        key=f"jpc_y_{i}",
                                        label_visibility="visible" if i==0 else "collapsed")
            with c4:
                pt["z"] = st.text_input("Z 標高 (m)" if i==0 else "Z (m)",
                                        value=pt["z"], placeholder="52.340",
                                        key=f"jpc_z_{i}",
                                        label_visibility="visible" if i==0 else "collapsed")
            with c5:
                if i == 0: st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
                if st.button("✕", key=f"del_jpc_{i}", disabled=len(pts)==1):
                    del_idx = i

        if del_idx is not None:
            pts.pop(del_idx); st.rerun()
...
<div class='rc-sub'>{fmt_decimal(lat_dd)}°</div></div>""", unsafe_allow_html=True)
                    with rc2:
                        st.markdown(f"""<div class='rc'>
<div class='rc-lbl' style='color:#10b981'>経度 LON</div>
<div class='rc-val'>{format_angle(lon_dd,FMT)}</div>
<div class='rc-sub'>{fmt_decimal(lon_dd)}°</div></div>""", unsafe_allow_html=True)
                    with rc3:
...
<div class='rc-sub'>{sub}</div></div>""", unsafe_allow_html=True)
                    ...
                    tooltip_txt = (f"Z={Zv:.3f}m / N={N:.4f}m / h={ellH:.3f}m"
                                   if ellH is not None else fmt_decimal(lat_dd))
                    map_rows.append({"name":pt["name"],"lat":lat_dd,"lon":lon_dd,"tooltip":tooltip_txt})
                    csv_rows.append(
                        f"{pt['name']},{Xv},{Yv},"
        ...
        for i, pt in enumerate(pts2):
            ...
            with c0:
                st.markdown(f"<div style='padding-top:{'32' if i==0 else '8'}px;font-size:12px;font-weight:700;color:#64748b'>#{i+1}</div>",
                            unsafe_allow_html=True)
            with c1:
                pt["name"] = st.text_input("点名", value=pt["name"], key=f"ll_nm_{i}",
                                           label_visibility="visible" if i==0 else "collapsed")
            with c2:
                pt["lat"] = st.text_input(
                    f"緯度（{in_fmt_lbl.split('（')[0]}）" if i==0 else "緯度",
                    value=pt["lat"], placeholder=ph_lat,
                    key=f"ll_la_{i}",
                    label_visibility="visible" if i==0 else "collapsed")
            with c3:
                pt["lon"] = st.text_input(
                    f"経度（{in_fmt_lbl.split('（')[0]}）" if i==0 else "経度",
                    value=pt["lon"], placeholder=ph_lon,
                    key=f"ll_lo_{i}",
                    label_visibility="visible" if i==0 else "collapsed")
            with c4:
