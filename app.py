...
A. 国土地理院のAPIにアクセスしています。
- 初回取得は数秒かかりますが、同じ座標は **24時間キャッシュ** されるため2回目以降は即座に表示されます
- 取得失敗時は最大3回リトライします（間隔：1.5秒・3秒）
- 失敗した場合は Z標高をそのまま楕円体高として出力します
- CSV一括変換では2点目以降に0.3秒の間隔を設けてAPI制限を回避しています

---

**Q. 往復変換するとXY座標が数mm変わります**

A. 正常な動作です。緯度経度の小数桁数が不足すると丸め誤差が積み重なります。
往復変換時は出力フォーマットを **十進角度（DD.DDDDDDDD°）** に設定してください。
十進角度8桁で変換すると XY誤差は通常 3〜11mm 程度になります。
...
現在の公共測量・建設 ICT では **JGD2011 または JGD2024** を使用します。旧成果を最新測地系に変換する場合は国土地理院のセミ・ダイナミック補正も必要になる場合があります。

---

**Q. CSVをExcelで開いても文字化けしません**

A. 出力CSVは **UTF-8 BOM付き** で出力しています。全角点名・大文字・記号入り点名もExcelで正しく表示されます。点名にカンマが含まれる場合も自動的にダブルクォートで囲んで出力します。

---

**Q. 緯度経度の形式が混在したCSVを読み込めますか？**

A. はい。CSV一括変換の「緯度経度 → 平面直角」では行ごとに異なるフォーマット（十進角度・度分秒・度分秒圧縮・方位角・Gons）を自動判別します。結果テーブルの「判別FMT」列で各行の判別結果を確認できます。

---

**Q. 度分秒圧縮（DD.MMSSSSSS）が十進角度として判別されます**

A. 誤判定を防ぐため、度分秒圧縮は **小数部12桁以上** を判定条件としています。8桁以下の値（例：`140.55591438`）は十進角度として扱います。12桁以上（例：`140.555914380000`）で入力してください。
""")
