...
- **平面直角座標（X・Y・Z）** と **緯度・経度・楕円体高** の相互変換
- **GNSS測量機器の出力データ**（Leica・Trimble・Topcon 等）の座標変換
- **CSV一括変換**で複数点を一度に処理

**基本の流れ**

1. サイドバーで **座標系（系番号）・測地系・ジオイドモデル** を設定
2. **単点変換** タブで座標を入力 → 変換結果を確認
3. 複数点は **CSV 一括変換** タブで処理
""")
...
| **地図スタイル** | 航空写真 または 標準地図 |

**ジオイドモデルについて**
- Z標高（正標高）↔ 楕円体高（h）の変換に必要です
- `楕円体高 h = Z標高 + ジオイド高 N`
- 「ジオイド補正なし」を選ぶと Z標高 ≒ 楕円体高 として扱われます
""")
...
| 変換方向 | 用途 |
|---|---|
| **緯度経度 形式変換** | DD.DDDDDDDD° ↔ DD°MM′SS.SSS″ など形式だけ変換 |
| **平面直角 → 緯度経度** | X・Y・Z 標高 を入力 → 緯度・経度・楕円体高 を出力 |
| **緯度経度 → 平面直角** | 緯度・経度・楕円体高 を入力 → X・Y・Z 標高 を出力 |
...
| 十進角度 | `39.61921024` |
| 度分秒 | `39°37′09.157″` |
| 度分秒圧縮 | `39.370915700000` （12桁以上） |
| 方位角 | `N39°37′09.157″` |
| Gons（グラード） | `44.0256gon` |

**操作のポイント**
- **＋ 点を追加** で複数点を同時に入力できます
- **⇄ X↔Y 入替** で X と Y を一括で入れ替えます（入力ミス修正に便利）
- **✕** で不要な行を削除します
""")
    ...
    with st.expander("📋 4. CSV一括変換の使い方"):
        st.markdown("""
**CSVファイルのフォーマット（ヘッダーなし・4列固定）**

| 変換方向 | A列 | B列 | C列 | D列 |
|---|---|---|---|---|
| **平面直角 → 緯度経度** | 点名 | X(m) | Y(m) | Z標高(m) |
| **緯度経度 → 平面直角** | 点名 | 緯度 | 経度 | 楕円体高(m) |
...
`A=点名, B=X(m), C=Y(m), D=Z標高(m), E=緯度, F=経度, G=楕円体高(m)`

**緯度経度 → 平面直角 の自動判別**

入力の緯度・経度は以下の5フォーマットを**自動判別**します（行ごとに混在OK）：
十進角度 / 度分秒 / 度分秒圧縮（12桁以上）/ 方位角 / Gons

**⚠️ 往復変換（JPC→LL→JPC）の注意**

出力された緯度経度をそのまま逆変換すると数mm程度の差が生じることがあります。
原因は緯度経度の小数桁数の丸め誤差です。
往復精度を上げるには出力フォーマットを **十進角度** にして小数8桁以上を使用してください。
""")

    # ── 5. 変換精度 ──
    with st.expander("🎯 5. 変換精度・仕様"):
        st.markdown("""
| 項目 | 仕様 |
|---|---|
...
| **縮尺係数** | m₀ = 0.9999（全系共通） |
| **往復誤差** | < 0.01 mm（同一数値の往復変換時） |
| **ジオイド高** | 国土地理院 API（JPGEO2024 / JPGEO2011）|
| **旧日本測地系** | Helmert 3パラメータ（Δx=−148, Δy=+507, Δz=+685 m）|

**JGD2024（測地成果2024）について**

令和6年（2024年）告示の最新測地系。GRS80楕円体・JGD2011と同一の準拠楕円体。
実用上は JGD2011 と同等の精度で利用可能（プレート運動補正が必要な場合を除く）。
""")
...
例：東京・神奈川・千葉・埼玉 → **9系**、北海道（旭川・函館）→ **11系**

---

**Q. ジオイド高の取得に時間がかかることがあります**

A. 国土地理院のAPIにアクセスしています。初回は数秒かかりますが、
同じ座標は **24時間キャッシュ** されるため2回目以降は即座に表示されます。

---

**Q. 往復変換するとXY座標が数mm変わります**

A. 正常な動作です。緯度経度の小数桁数が不足すると丸め誤差が生じます。
...
ただし現在の公共測量では **JGD2011 または JGD2024** を使用します。

---

**Q. CSVの変換結果をExcelで開くと文字化けします**

A. CSVはUTF-8（BOMなし）で出力しています。Excelで開く場合は
「データ」→「テキストファイルから」→ エンコード「UTF-8」を指定してください。
""")

    # ── 7. 他社機ローカライゼーションデータ変換 ──
    with st.expander("🔄 7. 他社機ローカライゼーションデータ変換（3DOffice対応）", expanded=False):
        st.markdown("""
### 🎯 ユースケース

**他社製 GNSS ローバーで取得した座標（緯度・経度・楕円体高）を
EarthBrain の 3DMG 等でローカライゼーションとして活用したい場合**に使用します。

> 例：Leica / Trimble / Topcon 等のローバーで観測した基準点データを
...
3DOffice・EarthBrain の 3DMG は度分秒圧縮形式（DD.MMSSSSSS）での入力が必要です。
...
> 💡 **ポイント**
> 複数の基準点を入力することで、ローカライゼーションの精度が向上します。
> 最低 1 点（推奨 3 点以上）の基準点データを用意してください。
""")
