...
tab1, tab2, tab3, tab4 = st.tabs(["📍 単点変換", "📋 CSV 一括変換", "ℹ️ 系番号一覧", "❓ 使い方"])
...
1. サイドバーで **座標系（系番号）・測地系・ジオイドモデル** を設定
2. **単点変換** タブで座標を入力 → 変換結果・CSV出力を確認
3. 複数点は **CSV 一括変換** タブでまとめて処理
""")

    # ── 2. サイドバー設定 ──
    with st.expander("⚙️ 2. サイドバーの設定項目"):
        st.markdown("""
| 設定項目 | 説明 |
|---|---|
| **座標系（系番号）** | 公共測量の平面直角座標系 1〜19系。現場の都道府県に合わせて選択 |
| **測地系** | JGD2024（推奨）/ JGD2011 / JGD2000 / WGS84 / 旧日本測地系 |
| **ジオイドモデル** | JPGEO2024（推奨）/ JPGEO2011 / 補正なし |
| **地図スタイル** | 航空写真（Esri World Imagery）または 標準地図（OpenStreetMap） |

**ジオイドモデルについて**

`楕円体高 h = Z標高 + ジオイド高 N`

- Z標高（正標高）と楕円体高（h）の相互変換に必要です
- **JPGEO2024**（令和6年度公開・最新モデル）を推奨。国土地理院APIに自動アクセスします
- 初回取得は数秒かかりますが、同じ座標は **24時間キャッシュ** されます
- 取得に失敗した場合は Z標高をそのまま楕円体高として出力します（⚠️警告表示）
- 「補正なし」を選ぶとジオイド高 N=0 として計算するため Z標高 = 楕円体高 として扱います
""")

    # ── 3. 単点変換 ──
    with st.expander("📍 3. 単点変換の使い方"):
        st.markdown("""
**変換方向の選択（左から順に表示）**

| 変換方向 | 用途 | CSV出力形式 |
|---|---|---|
| **緯度経度 形式変換** | 5形式を相互変換（座標系変換なし） | 点名, 緯度, 経度, 楕円体高 |
| **平面直角 → 緯度経度** | X・Y・Z標高 → 緯度・経度・楕円体高 | 点名, 緯度, 経度, 楕円体高 |
| **緯度経度 → 平面直角** | 緯度・経度・楕円体高 → X・Y・Z標高 | 点名, X, Y, Z標高 |

**緯度・経度の入力フォーマット（5形式自動判別）**

| フォーマット | 入力例 | 判別条件 |
|---|---|---|
| 十進角度 | `39.61921024` | 通常の小数 |
| 度分秒 | `39°37′09.157″` | °′″記号を含む |
| 度分秒圧縮 | `39.370915700000` | 小数部12桁以上 |
| 方位角 | `N39°37′09.157″` | 先頭がN/S |
| Gons（グラード） | `44.0256gon` | 末尾にgon/g/gr |

> ⚠️ **注意**：十進角度と度分秒圧縮の誤判定を防ぐため、度分秒圧縮は **小数部12桁以上** を必須としています。8桁以下の値は十進角度として扱います。

**操作のポイント**
- **＋ 点を追加** で複数点を同時に入力できます（点数制限なし）
- **⇄ X↔Y 入替** で全点の X と Y を一括入れ替え（入力ミス修正に便利）
- **✕** で不要な行を削除 / **🗑 全クリア** で全点リセット
- CSV出力は **UTF-8 BOM付き** のためExcelで直接開いても文字化けしません
""")

    # ── 4. CSV一括変換 ──
    with st.expander("📋 4. CSV一括変換の使い方"):
        st.markdown("""
**入力CSVのフォーマット（ヘッダーなし）**
...
- 1行目からデータ（ヘッダー行不要）
- 5列目以降は無視
- 変換方向を切り替えるとファイルは自動リセットされます

**出力CSVのフォーマット（7列）**

| A | B | C | D | E | F | G |
|---|---|---|---|---|---|---|
| 点名 | X(m) | Y(m) | Z標高(m) | 緯度 | 経度 | 楕円体高(m) |

結果テーブルには **ジオイド高N(m)** 列も表示されます（CSV出力には含まれません）

**緯度経度の自動判別（緯度経度 → 平面直角 時）**

5フォーマットを自動判別します（行ごとに混在OK）：
十進角度 / 度分秒 / 度分秒圧縮（12桁以上）/ 方位角 / Gons

**⚠️ 往復変換（平面直角→緯度経度→平面直角）の注意**

出力した緯度経度をそのまま逆変換すると XY に数mm の差が生じることがあります。
原因は緯度経度の小数桁数の丸め誤差です。
往復精度を上げるには出力フォーマットを **十進角度（DD.DDDDDDDD°）** にして入力してください。
""")
...
タブ上部の **「緯度経度 形式変換」** を選択します。

---

**STEP 3｜入力フォーマットを合わせる**

ローバーの表示・出力形式に合わせて入力フォーマットを選択してください。
...
| **準拠楕円体** | GRS80（JGD2024・JGD2011・JGD2000）／WGS84楕円体（WGS84）※実用上同等 |
| **縮尺係数** | m₀ = 0.9999（全系共通） |
| **対応座標系** | 公共測量 平面直角座標系 1〜19系 |
| **往復変換誤差** | < 0.01 mm（同一数値を往復変換した場合） |
| **ジオイド高** | 国土地理院 API（JPGEO2024 / JPGEO2011）・24hキャッシュ |
| **旧日本測地系変換** | Helmert 3パラメータ（Δx=−148, Δy=+507, Δz=+685 m）|
| **高さ小数精度** | Z標高・楕円体高 小数4桁（0.1mm精度）|

**測地系について**

| 測地系 | 概要 |
|---|---|
| **JGD2024**（推奨） | 令和6年（2024年）告示。準拠楕円体はGRS80。最新の公共測量成果に対応 |
| **JGD2011** | 東日本大震災後に制定（平成23年）。準拠楕円体はGRS80。JGD2024と同一の楕円体パラメータを使用 |
| **JGD2000** | 2002〜2011年の成果に使用 |
| **WGS84** | GPS/GNSS の基準座標系。GRS80と楕円体定数がほぼ同じで実用上の差は無視できる |
| **旧日本測地系** | 2002年以前の旧成果。現在の公共測量では使用不可 |

**往復変換の誤差について**

「往復誤差 < 0.01 mm」は同一の数値を往復させた場合の**計算精度**です。
実運用では緯度経度の小数桁数不足による丸め誤差（数mm）が生じる場合があります。
往復変換時は **十進角度（小数8桁以上）** を使用してください。
""")

    # ── 7. よくある質問 ──
    with st.expander("💬 7. よくある質問（FAQ）"):
        st.markdown("""
**Q. 座標系（系番号）はどれを選べばいいですか？**

A. 現場の都道府県で決まります。「ℹ️ 系番号一覧」タブで確認できます。
代表的な例：

| 系番号 | 対象地域 |
|---|---|
| 9系 | 東京・神奈川・千葉・埼玉・福島・栃木・茨城・群馬・静岡 |
| 10系 | 青森・秋田・山形・岩手・宮城 |
| 11系 | 北海道（小樽・函館・旭川エリア） |
| 12系 | 北海道（札幌・帯広・釧路エリア） |

→ **対策：** 不明な場合は「ℹ️ 系番号一覧」タブで現場の都道府県を確認し、系番号をサイドバーで設定してください。

---

**Q. ジオイド高の取得に時間がかかります / 取得に失敗します**

A. 国土地理院のAPIにアクセスしているため、通信状況によって遅延・失敗が起きることがあります。
- 初回は数秒かかりますが、同じ座標は **24時間キャッシュ** されるため2回目以降は即座に表示されます
- 取得失敗時は最大3回リトライします（間隔：1.5秒・3秒）
- 最終的に失敗した場合は **Z標高をそのまま楕円体高として出力**します（⚠️警告表示）

→ **対策：**
- 時間をおいて再度試してください
- 通信環境を確認してください（モバイル回線・Wi-Fi の切り替え等）
- ジオイド補正が不要な場合はサイドバーの「ジオイドモデル」を **「補正なし」** に設定することで API アクセスをスキップできます

---

**Q. 往復変換するとXY座標が数mm変わります**

A. 正常な動作です。緯度経度の小数桁数が不足すると丸め誤差が積み重なるためです。
十進角度の桁数が不足すると逆変換時に数mm〜十数mm程度の誤差が生じることがあります。

→ **対策：** 往復変換を行う場合は出力フォーマットを **十進角度（DD.DDDDDDDD°）** に設定してください。桁数が多いほど誤差が小さくなります。

---

**Q. 旧日本測地系のデータを変換したい**

A. サイドバーの測地系で「旧日本測地系（Tokyo97）」を選択してください。
現在の公共測量・建設 ICT では **JGD2011 または JGD2024** を使用します。

→ **対策：** 旧日本測地系の成果を現在の測地系（JGD2011/JGD2024）で利用する場合は、本ツールによる座標変換（Helmertパラメータ）で概略的な変換ができますが、cm級の精度が必要な場合は国土地理院の **TKY2JGD 変換プログラム** を使用してください。

---

**Q. 緯度経度の形式が混在したCSVを読み込めますか？**

A. はい。CSV一括変換の「緯度経度 → 平面直角」では、行ごとに異なるフォーマット（十進角度・度分秒・度分秒圧縮・方位角・Gons）を自動判別して処理します。

→ **確認方法：** 結果テーブルの **「判別FMT」列** で各行に適用されたフォーマットを確認できます。意図しないフォーマットで判別された行は入力値を修正してください。

---

**Q. 度分秒圧縮（DD.MMSSSSSS）が十進角度として判別されます**

A. 誤判定を防ぐため、度分秒圧縮は **小数部12桁以上** を判定条件としています。
例：`140.55591438`（8桁）→ 十進角度として処理されます。

→ **対策：** 度分秒圧縮形式で入力する場合は **小数部を12桁以上**にしてください。
例：`140.555914380000`（14桁）→ 度分秒圧縮として正しく処理されます。

---

**Q. CSV出力のファイル名はどう設定しますか？**

A. 変換結果の下に表示される **「📄 ファイル名」** 入力欄に任意の名前を入力してからダウンロードしてください。

→ **対策：**
- ファイル名が空欄のままだとダウンロードボタンが無効になります
- 使用できる文字：大文字・小文字・数字・漢字・ひらがな・カタカナ・ハイフン・アンダースコア・スペース
- 使用できない文字（バックスラッシュ / コロン / アスタリスク / ? " < > | など）は自動で除去されます
- `.csv` の拡張子は自動で付与されます
""")
