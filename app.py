...
ジオイド高が取得できない場合、Z標高をそのまま楕円体高として出力します。<br>
...
| **他社機ローカライゼーション対応** | SC ローバー以外の GNSS データを 3DOffice 形式に変換 |
...
| **準拠楕円体** | GRS80（JGD2024・JGD2011・JGD2000・WGS84 共通） |
| **縮尺係数** | m₀ = 0.9999（全系共通） |
...
| 測地系 | 概要 |
|---|---|
| **JGD2024**（推奨） | 令和6年告示。GRS80楕円体。最新の公共測量成果に対応 |
| **JGD2011** | 東日本大震災後に制定。実用上はJGD2024と同等 |
| **JGD2000** | 2002〜2011年の成果に使用 |
| **WGS84** | GPS衛星の基準座標系。GRS80とほぼ同一 |
...
十進角度8桁での逆変換では XY誤差が通常 3〜11mm 程度になります。
...
→ **対策：** 旧成果を最新測地系で使用する場合は、座標変換後に国土地理院の **セミ・ダイナミック補正** も合わせて実施してください。変換だけでは地殻変動分のずれが残る場合があります。
