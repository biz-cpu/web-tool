...
# ── タイトル直下：系番号一覧 / 使い方 ──
_hdr_col1, _hdr_col2, _hdr_spacer = st.columns([1, 1, 6])
with _hdr_col1:
    with st.expander("ℹ️ 系番号一覧"):
        ...
        st.dataframe(_dfz, use_container_width=True, hide_index=True, height=680)
        st.markdown("""
---
**仕様**
...
""")
        with st.expander("🔢 2. 座標系（系番号）の選び方"):
            st.markdown("""
現場の都道府県で系番号が決まります。代表例：
...
不明な場合は「ℹ️ 系番号一覧」を展開して確認してください。
""")
        with st.expander("📐 3. 3DOffice 等 建設 ICT ソフトへの入力手順"):
            st.markdown("""
**STEP 1｜座標系・測地系を設定する**
